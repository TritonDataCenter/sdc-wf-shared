...
function getVMs(job, callback) {
    ...
    if (Object.keys(tags).length === 0 && vms.length === 0 &&
        !job.params.fwapiAllVMs) {
        return callback(null, 'No tags or VMs to get');
    }
    ...
    job.log.info('listing VMs: %s', filterTxt);
    ...
    return vmapi.listVms({ query: filterTxt }, function (err, vmList) {
        ...
        if (job.params.task && job.params.task === 'provision') {
            // Remove the old entry from VMAPI if it exists - it will be
            // incomplete since provisioning hasn't finished
            vmList = vmList.filter(function (vm) {
                return (vm.hasOwnProperty('uuid') &&
                    vm.uuid != job.params.vm_uuid);
            });
        ...
        vmList.forEach(function (vm) {
            if (!vm.hasOwnProperty('nics') || vm.nics.length === 0) {
                job.log.info(vm, 'No nics in vm: not adding to remote VMs');
                return;
            }
        ...
        job.params.fwapiServers = Object.keys(servers).filter(function (uuid) {
            return (uuid !== 'null');
        });
        ...
        job.log.info({ matchingVMs: remoteVMs,
            servers: job.params.fwapiServers }, 'firewall VM data retrieved');
...
function resolveFirewallData(job, cb) {
    ...
    job.log.info({ query: params }, 'Resolving firewall data from FWAPI');
